    ...
    def evaluate(self, idx1: int, idx2: int) -> str:
        ...
        if phone_match is True:
            ...
            if name_match is False:
                # EXCEPTION: If Gov IDs also match, this is NOT a family member —
                # it is the same person with a name typo AND a changed phone.
                # Gov ID supremacy overrides the family guard.
                gov_match_early = self._gov_ids_match(idx1, idx2)
                if gov_match_early is not True:
                    n1 = self._get_best_name(idx1)
                    n2 = self._get_best_name(idx2)
        ...
        if email_match is True:
            name_match = self._names_match(idx1, idx2)
            if name_match is False:
                # EXCEPTION: Gov IDs match → same person with changed email, not family.
                gov_match_early = self._gov_ids_match(idx1, idx2)
                if gov_match_early is not True:
                    n1 = self._get_best_name(idx1)
                    n2 = self._get_best_name(idx2)
        ...
        # ── Rule 14: Gov ID match → MERGE (with fuzzy name confirmation) ─
        gov_match = self._gov_ids_match(idx1, idx2)
        if gov_match is True:
            # When Gov ID matches, use a LOWER name threshold (GOV_ID_NAME_CONFIRM_THRESHOLD = 70%)
            # because the ID is already a strong signal. This catches Type 2 fuzzy dupes
            # where the name has a typo AND contacts were changed.
            n1 = self._get_best_name(idx1)
            n2 = self._get_best_name(idx2)
            if n1 and n2:
                name_sim = _name_similarity(n1, n2)
                if name_sim >= GOV_ID_NAME_CONFIRM_THRESHOLD:
                    return "merge"   # Gov ID + name similar enough = same person
                if name_sim < 40:    # Names are completely different — suspicious
                    self._flag(idx1, idx2, "Rule 14",
                               f"Gov ID match but names too different ('{n1}' vs '{n2}', sim={name_sim:.0f}%) — "
                               f"possible shared account or data error → not merging")
                    return "block"
                # In the middle (40–69%): ambiguous — let fuzzy passes decide
                return "skip"
            # No name data at all
...
class DeduplicationEngine:
    ...
    def execute(self) -> pd.DataFrame:
        ...
        # ════════════════════════════════════════════════════════
        # PASS 3: Composite key match (FULL name + address/location ONLY)
        # ════════════════════════════════════════════════════════
        # SAFETY RULES:
        #   (a) DISABLED when email, phone, or Gov IDs are present.
        #       If strong identifiers exist, we must require them to agree.
        #       name+address alone can match roommates or family members
        #       who share the same address and have the same first name.
        #   (b) Uses ALL name columns joined (first+last) so that
        #       "John Smith" vs "John Doe" at the same address are NOT merged.
        #   (c) Requires explicit "merge" from the rule evaluator — not just non-block.
        # ════════════════════════════════════════════════════════
        self.log_callback("  [Pass 3] Composite full-name + location key (strict mode)...")
        if name_cols and not (email_cols or phone_cols or gov_id_cols):
            # Build a FULL-NAME key by joining ALL name columns.
            # e.g. first_name="John", last_name="Smith" → key="john smith"
            full_name_key = self.df[name_cols].apply(
                lambda row: " ".join(_norm(v) for v in row if _norm(v)), axis=1
            )

            secondary_candidates = block_cols + address_cols
            secondary = next((c for c in secondary_candidates if c in self.df.columns), None)

            if secondary and full_name_key.str.strip().ne("").any():
                composite = full_name_key + "||" + self.df[secondary].apply(_norm)
                groups = composite.groupby(composite).groups
                merged_p3 = 0
                for val, idxs in groups.items():
                    # Must have both a non-empty full-name part AND a non-empty address part
                    parts = val.split("||")
                    ...
                    for idx in idxs[1:]:
                        ...
                        if root1 == root2:
                            continue
                        # Require explicit "merge" — not just non-block
                        decision = rule_evaluator.evaluate(root1, root2)
                        if decision == "merge":
                            if uf.union(idxs[0], idx, reason="Composite full-name+location match"):
                                merged_p3 += 1
                if merged_p3:
                    self.log_callback(f"    Merged {merged_p3} records in Pass 3")
        elif name_cols:
            self.log_callback(
                "  [Pass 3] Skipped — contact IDs (phone/email/Gov ID) present. "
                "Name+address alone is insufficient when strong identifiers exist."
            )
        ...
        if name_cols:
            ...
            for idx1, idx2 in candidate_links:
                ...
                # "skip": business rules couldn't decide.
                # CRITICAL — Do NOT merge on name similarity alone (Rule 13).
                # We MUST have at least one strong corroborating field:
                #   • A Gov/Unique ID that both rows share (non-conflicting)
                #   • OR a matching phone number
                #   • OR a matching email address
                # If none of these corroborate, block the pair.
                has_gov_match     = False
                ...
                if not has_gov_match:
                    # Check phone
                    for col in phone_cols:
                        p1 = _norm_phone(self.df.loc[root1, col])
                        p2 = _norm_phone(self.df.loc[root2, col])
                        if p1 and p2 and p1 == p2:
                            has_phone_match = True
                            break
                    # Check email
                    if not has_phone_match:
                        for col in email_cols:
                            e1 = _norm_email(self.df.loc[root1, col])
                            e2 = _norm_email(self.df.loc[root2, col])
                            if e1 and e2 and e1 == e2:
                                has_email_match = True
                                break

                if not (has_gov_match or has_phone_match or has_email_match):
                    # Name alone is not sufficient — two different people can share a name.
                    # Without a corroborating strong field, do not merge.
                    blocked_p4 += 1
                    self.log_callback(
                        f"    [Pass 4] Blocked pair ({root1},{root2}): fuzzy name match but "
                        f"no corroborating field (phone/email/Gov ID) found."
                    )
                    continue

                # Has a corroborating field — safe to merge
                if uf.union(orig_idx1, orig_idx2, reason=f"Fuzzy name match ({name_sim:.0f}%) + corroborating field"):
                    merged_p4 += 1
    ...
    def _pass_multifield(
        ...
        for idx1, idx2 in candidate_links:
            ...
            # "skip": business rules couldn't decide.
            # Require at least one corroborating strong field — same rule as Pass 4.
            # A high multifield score driven mostly by name similarity is not enough;
            # two different people can share a common name.
            has_gov_match    = False
            has_gov_conflict = False
            has_phone_match  = False
            has_email_match  = False
            ...
            if uf.union(idx1, idx2, reason=f"Multifield match ({final_score:.0f}%) + corroborating field"):
                merged += 1
    ...
    def _build_golden_record(self, indices: List[int]) -> dict:
        ...
        # For phone and email columns, aggregate all unique, valid values separated by commas.
        for col in self.df.columns:
            is_phone = hasattr(self, 'phone_cols') and col in self.phone_cols
            is_email = hasattr(self, 'email_cols') and col in self.email_cols
            ...
            if is_phone or is_email:
                ...
                for sidx in indices:
                    ...
                    if _is_valid(v):
                        norm_v = _norm_phone(v) if is_phone else _norm_email(v)
                        if norm_v and norm_v not in seen_norm:
                            seen_norm.add(norm_v)
                            unique_vals.append(str(v).strip())
                if unique_vals:
                    golden[col] = ", ".join(unique_vals)
            else:
                # Standard backfill for other fields
